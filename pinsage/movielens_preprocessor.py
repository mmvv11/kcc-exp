...
"""
무비렌즈 데이터셋을 그래프 자료구조로 바꾸는 부분.

핀세이지는 기본적으로 이분 그래프 형태의 자료구조를 사용한다.

이는 meta path (movie - user - movie)를 통해 영화 간 연결 정보를 유저라는 중간 매개체로 담아내고, 결과적으로 영화 자체를 임베딩한다.

이 작업을 통해, meta path 정보를 가지고 있는 그래프 객체 g가 생성되고, 여기에 torch에서 바로 학습 가능한 형태의 feature vector를 생성한다.
"""
